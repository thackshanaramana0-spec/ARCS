...
ChainEncodeResult build_multicontig_pg(const std::vector<Read>& reads, CallData* call_out) {
    ...
    auto place_range = [&](const std::vector<int>& ids,
        ...
        auto index_contig = [&](uint32_t cid, uint32_t from, uint32_t to) {
            ...
            if (to < (uint32_t)K) return;
            uint32_t start = (from >= (uint32_t)(K - 1)) ? from - (uint32_t)(K - 1) : 0;
            if (use_minz && place_smask == 0) {
                // Minimizer-based indexing: only insert k-mers at sliding-window-minimum
                // positions (window = MINZ_W). Reduces index size ~W× while preserving the
                // overlap-detection guarantee: any overlap ≥ MINZ_W+K-1 shares a minimizer.
                // Consistent with the minimizer QUERY side so probes reliably hit indexed slots.
                int nk = (int)to - (int)start - K + 1;
                if (nk <= 0) return;
                std::vector<uint64_t> mkv(nk, ~0ULL), mkh(nk, ~0ULL);
                {   uint64_t v = 0; int valid = 0;
                    for (uint32_t p = start; p < to; ++p) {
                        v = ((v << 2) | encode_base(c[p])) & KMASK;
                        if (++valid >= K) {
                            int ki = (int)(p - (uint32_t)(K - 1)) - (int)start;
                            if (ki >= 0) { mkv[ki] = v; mkh[ki] = hash64(v); }
                        }
                    }
                }
                int W = MINZ_W;
                std::deque<int> dq; uint32_t last_ins = ~0u;
                for (int i = 0; i < nk; ++i) {
                    while (!dq.empty() && dq.front() <= i - W) dq.pop_front();
                    while (!dq.empty() && mkh[dq.back()] > mkh[i]) dq.pop_back();
                    dq.push_back(i);
                    if (i >= W - 1) {
                        int mi = dq.front();
                        uint32_t kpos = start + (uint32_t)mi;
                        if (kpos >= from && kpos != last_ins && mkv[mi] != ~0ULL) {
                            index.insert(mkv[mi], ((uint64_t)cid << 32) | kpos);
                            last_ins = kpos;
                        }
                    }
                }
    ...
    if (getenv("ARCS_MERGE_OFF") == nullptr) {
        ...
        // Minimizer-based merge indexing: index only sliding-window-minimum k-mers (window W).
        // Reduces KmerIndex size ~W× with deterministic coverage guarantee vs random smask.
        // ARCS_MERGE_MINZ=W enables (W 2-32). Default OFF (bit-identical to original).
        int merge_minz_w = 0;
        if (const char* s = getenv("ARCS_MERGE_MINZ")) { int v = atoi(s); if (v >= 2 && v <= 32) merge_minz_w = v; }
        ...
        for (int pass = 0; pass < MERGE_PASSES; ++pass) {
            ...
            else if (merge_minz_w > 0) est_post = est_post / (size_t)merge_minz_w + 1024;
