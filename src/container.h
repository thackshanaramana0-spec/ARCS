...
// lzma_compress/lzma_decompress: LZMA-9 via liblzma (fallback: zlib-9).
// zstd_compress/zstd_decompress: zstd via libzstd (when ARCS_HAVE_ZSTD).
// arcs_compress/arcs_decompress: unified — default is LZMA-9 (best ratio on
//   chain-pg blobs which are small). Set ARCS_COMPRESS_ZSTD=1 to route large
//   blobs (>=64KB) to zstd-19 (may help on datasets with very large blobs).
//   arcs_decompress auto-detects LZMA vs zstd by magic bytes (backward-compat).
...
std::vector<uint8_t> zstd_compress(const std::vector<uint8_t>& data, int level = 19);
std::vector<uint8_t> zstd_decompress(const uint8_t* data, size_t len);

// Preferred entry points for new encode/decode paths:
std::vector<uint8_t> arcs_compress(const std::vector<uint8_t>& data, int lzma_level = 9);
