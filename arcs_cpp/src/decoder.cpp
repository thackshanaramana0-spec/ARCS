...
static std::vector<std::string> decode_names_blob(
    ...
    if (blob[0] == 0x03 && blob.size() >= 5) {
        ...
        if (5 + (size_t)t_len <= blob.size()) {
            auto traw = lzma_decompress(blob.data()+5, t_len);
            auto xyraw = lzma_decompress(blob.data()+5+t_len, blob.size()-5-t_len);
    ...
    if (blob[0] == 0x04 && blob.size() >= 2) {
        auto raw = lzma_decompress(blob.data() + 1, blob.size() - 1);
    ...
    if (blob[0] == 0x05 && blob.size() >= 5) {
        ...
        auto raw = lzma_decompress(blob.data() + 5, sl);
    ...
    if (blob[0] == 0x02 && blob.size() >= 2) {
        ...
        if (nb >= 1 && nb <= 64 && blob.size() >= hdr) {
            ...
            if (total == blob.size()) {
                ...
                for (int b = 0; b < nb; ++b)
                    th.emplace_back([&, b] {
                        auto r = lzma_decompress(blob.data() + off[(size_t)b], clen[(size_t)b]);
...
void ARCSDecoder::decode_wgs(const ARCSReader& rdr, FASTQWriter& out_writer) {
    ...
    auto genome_raw = lzma_decompress(genome_blob.data(), genome_blob.size());
    ...
    if (rdr.has_blob(BlobType::UNMAPPED)) {
        ...
        if (!unmapped_blob.empty()) {
            auto raw = lzma_decompress(unmapped_blob.data(), unmapped_blob.size());
...
void ARCSDecoder::decode_amplicon(const ARCSReader& rdr, FASTQWriter& out_writer) {
    ...
    if ((is_v7 || is_v7star) && !seq_blob.empty()) {
        ...
        if (flag == 0x02) {
            ...
            seq_raw = lzma_decompress(payload, payload_len);
        }
    } else if (!seq_blob.empty()) {
        seq_raw = lzma_decompress(seq_blob.data(), seq_blob.size());
    }
    ...
    if (is_v7 || is_v7star) {
        ...
        auto cr = lzma_decompress(cb.data(), cb.size());
        size_t nu = cr.size() / 4;
        counts.resize(nu);
    ...
    if (is_v7star) {
        ...
        auto perm_raw  = lzma_decompress(perm_blob.data(), perm_blob.size());
        ...
        auto q_blob = rdr.read_blob(BlobType::QUALITY_DATA);
        auto q_raw  = lzma_decompress(q_blob.data(), q_blob.size());
    ...
    // Used only when decoding archives produced by the old encoder.
    {
        ...
        auto q_raw  = q_blob.empty() ? std::vector<uint8_t>{} :
                      lzma_decompress(q_blob.data(), q_blob.size());
...
void ARCSDecoder::decode_wgs_mst(const ARCSReader& rdr, FASTQWriter& out_writer) {
    ...
    auto tree_blob  = lzma_decompress(tree_compressed.data(),  tree_compressed.size());
    auto delta_blob = lzma_decompress(delta_compressed.data(), delta_compressed.size());
    ...
    if (q_model_blob.size() > 1) {
        auto decompressed = lzma_decompress(q_model_blob.data(), q_model_blob.size());
...
void ARCSDecoder::decode_wgs_chain(const ARCSReader& rdr, FASTQWriter& out_writer) {
    ...
    auto vseq_blob = lzma_decompress(vseq_compressed.data(), vseq_compressed.size());
...
static std::string decompress_pg(const std::vector<uint8_t>& blob) {
    ...
    if (flag == 0x01) {
        auto raw = lzma_decompress(payload, plen);
...
void ARCSDecoder::decode_wgs_chain_pg(const ARCSReader& rdr, FASTQWriter& out_writer) {
    ...
    auto pos_raw  = lzma_decompress(pos_comp.data(), pos_comp.size());
    ...
    // ── 3. Decompress and parse AUX data ─────────────────────────────────────
    ...
    auto aux_raw  = lzma_decompress(aux_comp.data(), aux_comp.size());
    ...
    _dmark("reconstruct");
    // ── 5. Decode quality (using MSTSequenceDecoder, same as chain mode) ────────
    auto q_data      = rdr.has_blob(BlobType::QUALITY_DATA)
                       ? rdr.read_blob(BlobType::QUALITY_DATA) : std::vector<uint8_t>{};
    auto qm_comp     = rdr.has_blob(BlobType::QUALITY_MODEL)
                       ? rdr.read_blob(BlobType::QUALITY_MODEL) : std::vector<uint8_t>{};
    std::vector<uint8_t> q_model_raw;
    if (qm_comp.size() > 1) {
        auto dec = lzma_decompress(qm_comp.data(), qm_comp.size());
        q_model_raw = dec.empty() ? qm_comp : dec;
    } else {
        q_model_raw = qm_comp;
    }
    ...
    std::vector<std::string> names;
    if (rdr.has_blob(BlobType::NAMES))
        names = decode_names_blob(rdr.read_blob(BlobType::NAMES), n);
    while (names.size() < n)
        names.push_back("read_" + std::to_string(names.size()));
    _dmark("names_decode");
    ...
    if (perm_mode == 1 && rdr.has_blob(BlobType::CHAIN_READ_PERM)) {
        auto perm_blob = rdr.read_blob(BlobType::CHAIN_READ_PERM);
        auto praw = lzma_decompress(perm_blob.data(), perm_blob.size());
        perm.resize(n);
    ...
    } else if (plus_style == 2 && rdr.has_blob(BlobType::PLUS_LINES)) {
        auto pb = rdr.read_blob(BlobType::PLUS_LINES);
        auto raw = lzma_decompress(pb.data(), pb.size());
        plus_asm.resize(n);
        size_t pos = 0;
        for (size_t i = 0; i < n; ++i) {
