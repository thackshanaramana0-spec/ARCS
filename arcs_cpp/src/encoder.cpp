...
static std::vector<uint8_t> build_columnar_names(const std::vector<Read>& reads) {
    ...
    if (nm_par) {
        auto f04 = std::async(std::launch::async, [&]{ return lzma_compress(raw04, 9); });
        lz05 = lzma_compress(raw05, 9);
        lz04 = f04.get();
    } else {
        lz04 = lzma_compress(raw04, 9);
        lz05 = lzma_compress(raw05, 9);
    }
...
std::vector<uint8_t> ARCSEncoder::encode_names(const std::vector<Read>& reads) const {
    ...
    if (getenv("ARCS_NAMES_NOTOK") == nullptr && n > 0) {
        ...
        auto build03 = [&]() -> std::vector<uint8_t> {
            ...
            auto t_lz = lzma_compress(t_raw, 9);
            auto xy_lz = lzma_compress(xy, 9);
    ...
    if (nb <= 1) return lzma_compress(raw, 6);
    ...
    for (int b = 0; b < nb; ++b)
        th.emplace_back([&, b] {
            ...
            comp[(size_t)b] = lzma_compress(chunk, 6);
        });
...
void ARCSEncoder::encode_wgs_se(const std::vector<Read>& reads,
    ...
    // This is the same strategy SPRING uses (LSH clustering → position order).
    {
        ...
        auto q_bytes = lzma_compress(qual_raw, 9);
    ...
    auto unmapped_bytes = lzma_compress(unmapped_raw, 9);
    ...
    // Store read_len in first 4 bytes (big-endian) for decoder.
    {
        ...
        std::copy(pg_result.genome.begin(), pg_result.genome.end(),
                  genome_raw.begin() + 4);
        auto genome_bytes = lzma_compress(genome_raw, 9);
...
void ARCSEncoder::encode_amplicon(const std::vector<Read>& reads,
    ...
    auto count_bytes = lzma_compress(count_raw, 9);
    ...
    auto qual_bytes = lzma_compress(flat_qual, 9);
    ...
    auto perm_bytes = lzma_compress(perm_raw, 9);
...
std::vector<uint8_t> ARCSEncoder::bsc_compress_raw(
    ...
    auto lzma_out = lzma_compress(raw, 9);
...
std::vector<uint8_t> ARCSEncoder::bsc_decompress_raw(
    ...
    if (flag == 0x01) {
        // LZMA fallback
        return lzma_decompress(payload, payload_len);
    }
...
void ARCSEncoder::encode_wgs_mst(const std::vector<Read>& reads,
    ...
    auto tree_compressed = lzma_compress(mst_result.tree_bytes, 9);
    ...
    if (L > 0 && L <= 50 && !reads.empty()) {
        ...
        auto colmaj_bytes = lzma_compress(colmaj, 9);
    ...
    auto q_model_blob = lzma_compress(mst_result.quality_model, 9);
...
void ARCSEncoder::encode_wgs_chain_pg(const std::vector<Read>& reads,
    ...
    auto pos_blob = lzma_compress(pos_raw, 9);
    ...
    auto aux_blob = lzma_compress(aux_raw, 9);
    ...
    if (surp_order < 1) surp_order = 1; else if (surp_order > 16) surp_order = 16;
    {
        ...
        if (!var_len && !hi_phred) {
            ...
            auto qmb_plain  = lzma_compress(qres_plain.second, 9);
        ...
        if (try_surp && !var_len) {
            ...
            auto qmb_surp = lzma_compress(qres_surp.second, 9);
    ...
    if (enc_par) { pg_blob = fut_pg.get(); name_blob = fut_names.get(); }
    ...
    if (!order_free) {
        ...
        if (order_col >= 0) {
            ...
            perm_blob = lzma_compress(praw, 9);
            perm_mode = 1;
    ...
    if (plus_style == 2) {
        ...
        plus_blob = lzma_compress(std::vector<uint8_t>(joined.begin(), joined.end()), 9);
    }
...
void ARCSEncoder::encode_wgs_chain(const std::vector<Read>& reads,
    ...
    auto vseq_compressed  = lzma_compress(result.vseq_bytes,  9);
    auto delta_compressed = lzma_compress(result.delta_bytes, 9);
    auto& chain_starts_raw = result.chain_starts;
    auto q_model_blob = lzma_compress(result.quality_model, 9);
    auto name_bytes = encode_names(reads);
...
EncodeProgress ARCSEncoder::compress(const std::string& input_path,
    ...
    // input to avoid bias from coordinate-sorted BAM conversion order.
    {
        ...
        ARCSHeader hdr;
        hdr.flags     = 0;
        hdr.flags    |= (akc.use_bsc && !params_.use_chain_pg && !params_.use_chain) ? 0x10 : 0;
